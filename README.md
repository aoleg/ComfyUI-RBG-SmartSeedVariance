...
- While the accordion checkbox is off the extension is completely inert: the CFG-denoiser callback is only registered for the duration of an enabled run and is removed again afterwards, so nothing is logged and no conditioning is touched. Each armed run is also bound to its own processing object, so sampling started by other extensions (ADetailer, upscalers, and anything else that calls `process_images()` internally) is left alone.
